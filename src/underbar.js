(function() {
  ...
  _.uniq = function(array, iterator) {
    ...
    if (typeof iterator === 'function') {
      for (let i = 0; i < array.length; i ++) {
        if (!modifiedArray.includes(iterator(array[i]))) {
          results.push(array[i]);
        }
        modifiedArray.push(iterator(array[i]));
      }
    } else {
      for (let i = 0; i < array.length; i ++) {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    let copy = collection; //collection vs collection.slice();
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if (result.hasOwnProperty(stringArg)) {
        returnedResult = result[stringArg];
      } else {
        returnedResult = func.apply(this, arguments);
        result[stringArg] = returnedResult;
      }
  ...
  _.shuffle = function(array) {
    ...
    for (let i = 0; i < copy.length; i ++ ) {
      ...
      while (alreadyGenerated.includes(randomNumber)) {
        randomNumberGenerator();
      }
